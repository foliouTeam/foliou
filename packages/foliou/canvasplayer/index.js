define(['jquery'], function ($) {
    var canvasframe = function (container, options) {
        ...
        this.getRealIndex = function(frame){
            var relindex = frame;
            if(framesArray instanceof Array){
                relindex = parseInt(frame)-1;
            }
            if(!!self.prefix){
                relindex = self.prefix + frame;
            }
            return relindex;
        }
        this.frameIsLoaded = function (frame) {
            
            var relindex = this.getRealIndex(frame)
            if (typeof (framesArray[relindex]) != 'undefined' && framesArray[relindex]) {
                return true;
            } else {
                return false;
            }
        ...
        this.drawFrames = function (frame) {
            ...
            var relindex = this.getRealIndex(frame);
            self.ctx.drawImage(framesArray[relindex], 0, 0);
        }
