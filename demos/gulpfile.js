...
var ztGulp = {
    tasks:{
        ...
        buildcss:function(){
            ...
            return gulp.src(src + '/!(lib|modules|node_modules)/**/*.{css,scss,sass,less}')
                ...
                .pipe( gulpif(!!argv.m, rename({suffix: '.min'})) )
                .pipe(gulp.dest(outpath + filen))
                .pipe(reload({ stream: true }));
        },
        imagemin:function(){
            var filen = "";
            return gulp.src(src + '/!(lib|modules|node_modules)/**/*.{png,jpg,gif,ico}')
                .pipe(through.obj(function(file, enc, cb) {
        ...
        copy:function(){
                ...
                ///二级目录以上的
                let task2 = gulp.src(src + "/!(lib|modules|node_modules|js|css|images)/**/*.*")
                    .pipe(through.obj(function(file, enc, cb) {
                        filen = file.relative.split(path.sep);
                        filen.pop();
                        filen = '/' + filen.join("/");
        ...
        server:function(){
            ...
            var proxyTargetKey = ['/api', '/login' , '/user', '/project', '/upload','/action.php']; //默认为数组，参数为空则不使用
            var proxyTarget = "http://act.balls.web.ztgame.com/city2018/"; //使用代理
            // 代理配置, 实现环境切换
            var middleware = proxyMiddleware(proxyTargetKey, {target: proxyTarget, changeOrigin: true});
            browserSync.init({
                server: {
                    baseDir: outpath,
                    index: "index.html",
    ...
    setTasks:function(){
        ...
        gulp.task('dev', function(done) {
            condition = false;
            runSequence(
                ['clean'], ['clear'], ['copy'], ['compile'], ['htmlmin'], ['imagemin'], ['server'],
                done);
        });
